...
- `references/dispatch-boundary-evidence.md` — while writing the prompt and while reading the return: withhold any figure the task exists to re-derive, resolve each inbound flag's named consumer against the partition key, attribute the cumulative diff yourself, and recover a reply that did not route.
