...
| 18 | DELEGATED output-size measurement (`measure_files.py`) missing after dispatch (`agent-orchestration-delegated.md` §1.4) | BLOCKER | Orchestration between dispatches |
...
| 63 | Token Saver large-file ladder not applied — `context.token_saver: true` AND (over-ceiling task without `1M-exception`; OR Warn+ Required Context file with no backlog item; OR a `read`-reason Critical wrongly flagged `1M-exception`; OR a `1M-exception` task on a Sonnet/Haiku agent without override note; OR a runner-read generated artifact past the token/byte/line read gate without a Multi-Part split) (`task-content-fidelity.md` §9.A.8) — no-op when Token Saver is off | ERROR (read-Critical mis-flag / over-ceiling / artifact split) · WARNING (missing backlog item / uncovered read gate) | Task Required Context + Notes for Agent ([Token Saver Compliance Check](review-classification.md#token-saver-compliance-check)) |
| 64 | Orchestrator consumes sub-agent verdict label without recomputing from reported finding counts — systematic under-classification risk (`agent-orchestration-delegated-Part-2-DispatchMechanicsAndReturns.md` §1.16.1) | ERROR | Orchestration synthesis step; rollup tables |
| 65 | Orchestrator accepts cross-file control-flow claim ("symbol X never used → feature Y is broken") without tracing the full consumer call path — false-positive over-classification risk (`agent-orchestration-delegated-Part-2-DispatchMechanicsAndReturns.md` §1.16.2) | WARNING | Orchestration finding acceptance; release-signoff verdicts |
...
| 90 | Removal deliverable enumerated, not derived — no Creator-role member and no explicit retained-creator statement (`scaffolding-hygiene-Part-2-DerivationAndParallelism.md` §13.1 + §13.2) | BLOCKER | Deliverables section vs sweep-derived deletion set |
...
| 96 | Task whose steps predictably breach a binding rule, with no halt-and-report instruction in its Notes for Agent (`agent-orchestration-delegated-Part-3-CrossCuttingDispatchDiscipline.md` §1.25) | ERROR | Task file Notes for Agent |
| 97 | DELEGATED spawn prompt opening with a session-scoped identity frame ("You are a task-runner in session {session-id}") and no task-scoped clause in the same sentence — the framing measured to override a correctly-stated single-task constraint (`agent-orchestration-delegated-Part-3-CrossCuttingDispatchDiscipline.md` §1.23) | ERROR | Orchestration spawn prompts (opener framing) |
| 98 | DELEGATED spawn prompt naming the task scope in fewer than three positions — the mechanical, grep-countable tally across Opener / Hard-constraint block / Return instructions (`agent-orchestration-delegated-Part-3-CrossCuttingDispatchDiscipline.md` §1.23) | BLOCKER | Orchestration spawn prompts (position count) |
| 99 | Structurally-consumed output specified only as prose + illustrative code block — no template pointer, no heading/column checklist (`agent-orchestration-delegated-Part-3-CrossCuttingDispatchDiscipline.md` §1.24) | ERROR | Task file Expected Output |
| 100 | Verification command that mutates the artifact it checks, unattributed, where that artifact is a runner's declared output (`agent-orchestration-delegated-Part-3-CrossCuttingDispatchDiscipline.md` §1.26) | ERROR | Task file Verification Commands |
| 101 | Task file's own Verification Commands invoking a project tool by bare name, in a project declaring an isolated environment, with no interpreter path present (`agent-orchestration-delegated-Part-3-CrossCuttingDispatchDiscipline.md` §1.27) | ERROR | Task file Verification Commands |
| 102 | DELEGATED spawn prompt's dispatched command invoking a bare interpreter rather than the project-relative path (`agent-orchestration-delegated-Part-3-CrossCuttingDispatchDiscipline.md` §1.27) | WARNING | Orchestration spawn prompts / ENVIRONMENT DISCIPLINE block |
...
| 105 | Plan with a task whose declared Output is under `.claude/**` and no Orchestration declares the expected permission round-trip, or one task batching edits across multiple config files with no smallest-coherent-set scoping (`scaffolding-hygiene-Part-2-DerivationAndParallelism.md` §14) | ERROR | Orchestration Execution Strategy / Task file Output |
...
| 111 | State-asserting, hedge, or cited-authority-conclusion row compressed without re-derivation — an EI/Deliverable/exit-criterion row asserts a state, carries a source hedge, or cites another artifact's conclusion as authority, with no measured re-check at scaffold close (`ei-source-promise-integrity.md` §10.4 + `ei-fidelity.md` §4.5 + `scaffolding-hygiene.md` §12.3) | BLOCKER (state/authority) · ERROR (hedge) | EI body / exit criteria / Signoff anchors |
| 112 | Vacuous After-gate — the measured pre-edit value already satisfies the stated post-edit expectation, so the gate passes with zero work done (`verification-task-authoring.md` §10) | ERROR | Task file Verification Commands After block |
| 113 | Missing pre-edit baseline — an After-gate with no recorded pre-edit value beside its expectation, so vacuity is unknowable (`verification-task-authoring.md` §10) | WARNING | Task file Verification Commands After block |
| 114 | BRE/ERE mismatch — a `grep` invocation without `-E` whose pattern contains `(`, `)` or `\|`, so the gate returns 0 unconditionally and can never fail (`scripts/lint_verification_gates.py`) | ERROR | Task file Verification Commands gate command |
| 115 | Self-matching sweep — an expect-0 tree-wide sweep in a task whose declared Output names new files, with no path exclusion for that family; returns non-zero on correct work and halts the chain (`scripts/lint_verification_gates.py`) | ERROR | Task file Verification Commands sweep gate vs task Output |
| 116 | Stale-ownership assertion — an absence-assertion naming a file plus a §-anchor, where the same plan routes that § out of that file; passes automatically for the wrong reason (`scripts/lint_verification_gates.py`) | ERROR | Task file Verification Commands absence-assertion vs plan §-routing |
| 117 | Substring gate over own vocabulary — a gate greping a generated report for a bare token the report's own template emits, so a passing report halts its own downstream chain (`scripts/lint_verification_gates.py`) | WARNING | Task file Verification Commands gate vs generated report template |
| 118 | Contradicted Before-baseline — a Before-block stated baseline that disagrees with the measured live value (`scripts/lint_verification_gates.py`) | ERROR | Task file Verification Commands Before block |
| 119 | Catalog row with no loader — a row citing a reference that the handler loads only as a lead-side conditional, while the check is delegated to a spawned reviewer whose prompt names neither the reference nor the condition; the row is indexed and never evaluated (`agent-orchestration-delegated-Part-3-CrossCuttingDispatchDiscipline.md` §1.29.2) | ERROR | Handler spawn prompts vs the rows citing that reference |
| 120 | Context subtotal gap closed by a prose qualifier rather than an addend — `sum(rows) + output + working` deviates from the header's `Estimated Tokens` by more than the ±0.5K a rounded header implies, and the file carries a qualifier in place of the arithmetic; or two mutually inconsistent derivations appear in one task file (`templates/task-file.md` Context subtotal identity) | ERROR | Task file Required Context subtotal vs header Estimated Tokens |
| 121 | Anchor enumerates fewer outcome branches than its task produces — a Signoff anchor, exit criterion or EI gate accepts a strict subset of the terminal outcomes the owning task's Execution Steps define, so a correct execution FAILs it (`verification-task-authoring.md` §10.7 + `exit-criteria-fidelity.md` §16.10.6) | WARNING (ERROR when no accepted outcome remains for the branch the task produces) | Signoff Mechanical Anchor rows / exit criteria vs owning task Execution Steps |
| 122 | Anchor path unresolvable from its own declared cwd — a Signoff anchor row runs a path relative to a directory other than the one its table header declares, so the anchor fails unconditionally (`verification-task-authoring.md` §10.8 trap 4) | WARNING | Signoff Mechanical Anchor Checks table |
