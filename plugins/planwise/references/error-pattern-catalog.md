...
| 119 | Catalog row with no loader — a row citing a reference that the handler loads only as a lead-side conditional, while the check is delegated to a spawned reviewer whose prompt names neither the reference nor the condition; the row is indexed and never evaluated (`agent-orchestration-delegated.md` §1.29.2) | ERROR | Handler spawn prompts vs the rows citing that reference |
